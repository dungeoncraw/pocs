...
@ActiveProfiles("test")
@AutoConfigureWebMvc
public class AlivePostgres15ContainerTest {
    ...
    @Test
    void shouldStartPostgres15Container() {
    ...
    @Test
    void shouldVerifyPostgres15Version() throws Exception {
    ...
    @Test
    void shouldHandleMultipleAliveCallsAndVerifyStorage() {
        String url = "http://localhost:" + port + "/alive";

        // Make multiple calls
        ResponseEntity<NonUserEntity> response1 = restTemplate.getForEntity(url, NonUserEntity.class);
        ResponseEntity<NonUserEntity> response2 = restTemplate.getForEntity(url, NonUserEntity.class);

        assertThat(response1.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response2.getStatusCode()).isEqualTo(HttpStatus.OK);

        // Verify different UUIDs were generated
        assertThat(response1.getBody().getUuid()).isNotEqualTo(response2.getBody().getUuid());
        ...
        NonUserEntity entity1 = nonUserRepository.findById(response1.getBody().getId()).orElse(null);
        NonUserEntity entity2 = nonUserRepository.findById(response2.getBody().getId()).orElse(null);

        assertThat(entity1).isNotNull();
        assertThat(entity2).isNotNull();
        assertThat(entity1.getUuid()).isEqualTo(response1.getBody().getUuid());
        assertThat(entity2.getUuid()).isEqualTo(response2.getBody().getUuid());
    }
}
