...
object Commands:
  ...
  @main
  def build(@arg(doc = "Include drafts") drafts: Boolean = false): Unit =
    val configPath = os.pwd / ProjectPaths.Config.path
    if !os.exists(configPath) then
    ...
    val allParsedPosts = postFiles.flatMap { path =>
      try {
        val content = os.read(path)
        val (fm, body) = Markdown.parseFile(content)
        
        if fm.title == "Error Parsing Front Matter" then
          warnings += s"Failed to parse front matter in ${path.last}"
        ...
        case e: Exception =>
          errors += s"Error processing ${path.last}: ${e.getMessage}"
          None
      }
    }.toList
