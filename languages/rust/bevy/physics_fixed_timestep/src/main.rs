...
//! [this cheat book entry](https://bevy-cheatbook.github.io/fundamentals/fixed-timestep.html).
//!
//! ## Motivation
//!
...
//! and somewhere it is updated by a large amount that resulted from running the physics simulation multiple times.
...
//! See the [documentation of the light year crate](https://cbournhonesque.github.io/lightyear/book/concepts/advanced_replication/visual_interpolation.html)
...
/// A vector representing the player's input, accumulated over all frames that'd run
...
/// Spawn a bit of a UI text to explain how to move the player.
...
fn advance_physics(
    ...
    ) in query.iter_mut()
    {
        ...
        // Reset the input accumulator, as we are currently consuming all inputs that happened since the last fixed timestep.
        input.0 = Vec2::ZERO;
    }
}
